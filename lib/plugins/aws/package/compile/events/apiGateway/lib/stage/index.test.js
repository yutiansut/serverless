...
describe('#compileStage()', () => {
  ...
  it('should not apply any settings on external API Gateway', () => {
    serverless.service.provider.apiGateway = { restApiId: 'foosdfsdf' };
    return awsCompileApigEvents.compileStage().then(() => {
      expect(awsCompileApigEvents.apiGatewayStageLogicalId).to.equal(undefined);
    });
  });
